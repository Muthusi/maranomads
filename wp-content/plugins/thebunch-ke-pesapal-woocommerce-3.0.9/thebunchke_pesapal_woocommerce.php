				...
				if(!class_exists('WC_TheBunchKE_PesaPal_Pay_Gateway')){
					class WC_TheBunchKE_PesaPal_Pay_Gateway extends WC_Payment_Gateway{
						...
						public function generate_notification_id(){
							...
							if( $merchant->pesapal_consumer_key != $this->consumer_key || $merchant->pesapal_secret_key != $this->consumer_secret || $merchant->notification_id == 00000000-0000-0000-0000-000000000000){
									...
									<?php if($this->loadjquery || $this->apiVersion==25 || $this->apiVersion==26){ ?>
										...
										<script type="text/javascript">
											jQuery(document).ready(function() {
												...
												var reference = '<?php echo ($this->get_option('ppprefix')) ? strtoupper(str_replace(" ","",$this->get_option('ppprefix')))."-".$order->get_id() : $order->get_id(); ?>';
						...
						public function create_url($order_id){
							...
							if($access_token){
								...
								if(!$url){
									...
									$request->pesapalMerchantReference = ($this->get_option('ppprefix')) ? strtoupper(str_replace(" ","",$this->get_option('ppprefix')))."-".$order->get_id() : $order->get_id();
